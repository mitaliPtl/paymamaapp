                ...
                foreach ($tableHeads as $headInd => $head) {
                    ...
                    elseif ($head['label'] == 'customer_name') {
                        ...
                        $keyList[$head['label']] = $label_val;
                           ...
                           if($report['order_status'] == 'SUCCESS' || $report['order_status'] == 'success' ) {
                            $keyList[$head['label']] = $report['retailer_commission'];
                            /*
                            if($report['retailer_commission'] != '' or $report['retailer_commission'] != NULL){
                                $reportList[$repInd]['rt_commission'] = $report['retailer_commission'];
                                    
                            }else{
                                $packegeId=User::getPackageId($report['user_id']);
                                //print_r($packegeDetails);
                                $condition=array('service_id'=>$report['service_id'], 'pkg_id'=>$packegeId, 'operator_id'=>$report['operator_id']);
                                // print_r($condition);
                                $commissionDetails=PackageCommissionDetail::WHERE($condition)->get()->first();
                               // print_r(count($commissionDetails));
                                if($head['label'] == "rt_commission"){
                                    $keyList[$head['label']] = round(($report['total_amount']*@$commissionDetails->retailer_commission)/100, 2);  
                                    //$reportList[$repInd]['rt_commission'] =$label_val;
                                }
                                
                            }
                            */
                            }else{
                                $keyList[$head['label']] = 0;  
                            }
                            
                      }elseif ($head['label'] == "dt_commission") {
                        if($report['order_status'] == 'SUCCESS' || $report['order_status'] == 'success' ) {
                            $keyList[$head['label']] = $report['distributor_commission'];
                            /*
                            if($report['dt_commission'] != '' or $report['dt_commission'] != NULL){
                                $reportList[$repInd]['rt_commission'] = $report['distributor_commission'];
                                    
                            }else{
                                $condition=array('service_id'=>$report['service_id'], 'pkg_id'=>User::getPackageId($report['user_id']), 'operator_id'=>$report['operator_id']);
                                
                                $commissionDetails=PackageCommissionDetail::WHERE($condition)->get()->first();
                               
                                if($head['label'] == "dt_commission"){
                                    $keyList[$head['label']] = round(( $report['total_amount']*@$commissionDetails->distributor_commission)/100, 2);  
                                   // $reportList[$repInd]['dt_commission'] =$label_val;
                                }
                        ...
                        if($report['order_status'] == 'SUCCESS' || $report['order_status'] == 'success' ) {
                            $keyList[$head['label']] = $report['md_commission'];
                            /*
                            if($report['md_commission'] != '' or $report['md_commission'] != NULL){
                                $reportList[$repInd]['md_commission'] = $report['md_commission'];
                                ...
                                //print_r($report);
                                $commissionDetails=PackageCommissionDetail::WHERE($condition)->get()->first();
                                if($head['label'] == "md_commission"){
                                    $keyList[$head['label']] = round(($report['total_amount']*@$commissionDetails->md_commission)/100, 2);  
                                   // $reportList[$repInd]['md_commission'] =$label_val;
                                }
