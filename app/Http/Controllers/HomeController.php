...
class HomeController extends Controller
{
    ...
    $filename = "backup-.gz";
        ...
        $returnVar = NULL;
        $output  = NULL;
        $ht=exec($command, $output, $returnVar);
        return $ht;
    ...
    public function index(Request $request)
    {
       ...
       $pendingComplaints = Complaint::where('complaint_status', "PENDING")->get()->count();
        ...
        // return  $transaction['success'] = TransactionDetail::where('order_status', "SUCCESS")
    ...
    public function showInvoice($order, $subcharge, Request $request){
        ...
        if( ( $service_type == 'money_transfer' ) || ( $service_type == 'upi_transfer' )  || ( $service_type == 'aeps' ) || ( $service_type == 'ap' ) || ( $service_type == 'mini_statement' )  || ( $service_type == 'Mini_Statement' ) ){
            $surcharge = $subcharge;
            // $this->showInvoiceDMT($order, $subcharge, $transDtls);
            $ttl_amt=0;
            $tranDtls_gid=[];
            $user_id = $transDtls->user_id;
            ...
            $serviceid = $transDtls['service_id'];
            $amount = $transDtls['total_amount'];
            $trans_date = $transDtls['trans_date'];
            $aeps_balance = $transDtls['aeps_balance'];
            $aadharnumber = $transDtls['aadharnumber'];
            $order_id = $transDtls['order_id'];
            $transaction_id = $transDtls['transaction_id'];
            $client_reference_id = $transDtls['client_reference_id'];
            $rrnno = $transDtls['rrnno'];
            $bank_name = $transDtls['aeps_bank_id'];
            
            
               return view('modules.service_type.reciptDMT', compact('bank_name','rrnno','client_reference_id','transaction_id','trans_date','aeps_balance','aadharnumber','order_id','amount','serviceid','tranDtls', 'tranDtls_gid', 'surcharge', 'final_amt', 'fileName', 'user', 'ttl_amt'));
          //  return view('modules.service_type.reciptDMT', compact('amount','serviceid','tranDtls', 'tranDtls_gid', 'surcharge', 'final_amt', 'fileName', 'user', 'ttl_amt'));
            }
        }else {
            ...
            $amount = (isset($transDtls['basic_amount']) ) ?  $transDtls['basic_amount']  : 0;
