...
<div class="page-content container-fluid">
    <div class="card card-body" style="width:50%;">
        <h2 style="font-weight:700">RETAILER REGISTRATION</h2>
        <hr style="1px dotted red">

        <h4 class="text-danger mx-5">{{$error ?? ''}}</h4>

        <form action="/store_new_retailer" method="POST">
            @csrf

            <div class=" form-group">
                <label for="first_name">First name</label>
                <input type="text" class="form-control" id="first_name" required value="{{request()->input('first_name')}}" name="first_name">
            </div>
            <div class=" form-group">
                <label for="last_name">Last name</label>
                <input type="text" class="form-control" id="last_name" required value="{{request()->input('last_name')}}" name="last_name">
            </div>

            <div class=" form-group">
                <label for="first_name">Mobile Number</label>
                <input type="number"  class="form-control" required value="{{request()->input('mobile_no')}}" id="mobilenumber" name="mobile_no">
            </div>
            <div class=" form-group">
                <label for="last_name">Email Id</label>
                <input type="email" class="form-control" id="email_id" required value="{{request()->input('email_id')}}" name="email_id">
            </div>
            <div class=" form-group">
                <label for="last_name">Business Name</label>
                <input type="text" class="form-control" name="business_name" required value="{{request()->input('business_name')}}" >
            </div>

            <div class=" form-group">
                <label for="last_name">Fos</label>
                <select class="form-control" name="fos" >
                    @foreach($fos_users as $fos)
                    <option {{ request()->input('fos') == $fos->userId ? "selected" :""}}  value="{{$fos->userId}}">{{$fos->first_name}}</option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <center>
                    ...
                    <h3 class="text-success" id="">Retailer Created Successfully</h3>
                    ...
                    <div class="col-8">
                        <table id="recharge_resp-table" class="table" style="font-size: 18px">
                            ...
                            <tr>
                                <th class="text-red">
                                    Business Name <span class="colon-algin">:</span>
                                </th>
