            ...
            <div class="card-body">
                ...
                <div class="row">
                    ...
                    <div class="col-12 text-right">
                        <a type="button" href="{{ route('user_list') }}" title="Active Users" class="btn btn-primary btn-md " style="color:white;"> All Non-eKyc Users </a>
                    </div>
                    @else
                    <div class="col-12 text-right">
                    ...
                    <table id="user-table" class="table table-striped table-sm border is-data-table">
                        ...
                        <tbody>
                            @foreach($userList as $index => $user)
                                <tr>
                                    <td>{{ $index+1 }}</td>
                                    <td>{{ $user->username }} ({{ $user['ekyc']['business_name'] }})</td>
                                    <td>{{ $user->first_name }}</td>
                                    <td>{{ $user->mobile }}</td>
                                    <td>{{ $user->email }}</td>
                                    <td>{{ $user['parentuser']['username'] }} ({{ $user['parentuser']['ekyc']['business_name'] }})</td>
                                    <td>
                                    ...
                                    <td>{{ $user->createdDtm ? date('d/m/Y', strtotime($user->createdDtm)) :'' }}</td>
                                    <td>₹ {{ $user->min_balance }}</td>
                                    <td>₹ {{ $user->wallet_balance }}</td>
                                    <td>
                                    {{ $user->last_activity ?  $user->last_activity : '' }}
                                    {{-- $user->last_activity ?  substr($user->last_activity,0,10) : '' --}}
                                    </td>
                                    <td>{{ $user->last_login_ip ?  $user->last_login_ip : '' }}</td>
                                    <td class="label">
                                        <a href="{{ route('edit_userekyc',$user->userId) }}"
                                         class="{{ isset($user->nonkyc->complete_kyc) && $user->nonkyc->complete_kyc == '1' ?  'text-success' : (isset($user->nonkyc->complete_kyc) && $user->nonkyc->complete_kyc == '0' ? 'text-warning' : 'text-danger') }}">
                                            {{ isset($user->nonkyc->complete_kyc) && $user->nonkyc->complete_kyc == '1' ?  'APPROVED' : (isset($user->nonkyc->complete_kyc) && $user->nonkyc->complete_kyc == '0' ? 'PENDING' : 'NA') }}
                                        </a>
                                    </td>
                                    <td class="text-center">
                                        @if($user->activated_status == Config::get('constants.ACTIVE'))
