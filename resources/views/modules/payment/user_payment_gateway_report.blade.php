                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-12 text-right mb-2">
                            ...
                            @if(isset($total_amt['total_charges']) && $total_amt['total_charges'])
                            <button type="button" title="Total Amount" class="btn btn-light info-button btn-md mr-2">Total Charges : {{ $total_amt['total_charges'] }}</button>
                            @endif
                            ...
                            @csrf
                                ...
                                @foreach($filtersList as $i => $filter)
                                    <div class="filter-elements">
                                            ...
                                            @if($filter['name'] == "filter_payment_mode")
                                                <select name="{{ $filter['name'] }}" id="{{ $filter['id'] }}" class="form-control">
                                                    <option value="" selected>{{ $filter['label'] }}</option>
                                                    @foreach($payment_charges as $charges)
                                                        @if($charges->mode == $request->filter_payment_mode)
                                                            <option value="{{ $charges->mode }}" selected> {{ $charges->mode }}</option>
                                                        @else
                                                            <option value="{{ $charges->mode }}"> {{ $charges->mode }}</option>
                                                        @endif
                                                    @endforeach
                                                </select>
                                            @endif
                                    ...
                                    <tr>
                                        ...
                                        @foreach($reportTH as $i => $head)
                                            ...
                                            @elseif($head['label'] == 'transaction_status')
                                                <td class="{{ $report[$index][$head['label']] == 'SUCCESS' ? 'text-success' : ($report[$index][$head['label']] == 'TXN_SUCCESS' ? 'text-success' : ($report[$index][$head['label']] == 'PENDING' ? 'text-warning' : 'text-danger')) }}">
                                                @if($report[$index][$head['label']] == 'TXN_SUCCESS')
                                                    SUCCESS
                                                @elseif($report[$index][$head['label']] == 'TXN_FAILURE')
                                                    FAILED
                                                @else
