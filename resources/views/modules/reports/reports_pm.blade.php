                        ...
                        <div class="row">
                            <div class="col-12 col-sm-2">
                                <div class="form-group ">
                                    <label for="exampleInputEmail1" class="font-weight-bold">USERNAME/MOBILE</label>
                                    <input type="text" class="form-control" name="username_mobile" placeholder="username/mobile" value="{{request()->input('username_mobile')}}">
                                </div>
                            </div>
                    ...
                    <div class="col-12">
                        ...
                        <table id="retailerdatatable" class="table table-striped  table-bordered table-sm border ">
                            ...
                            <tbody>
                                ...
                                @foreach($tranDtls as $index => $trans)
                                <tr>
                                    <td >{{ $index+1 }}</td>
                                    ...
                                    @if($trans->payment_type == 'SERVICE')
                                    <td>{{ $trans->total_amount }}</td>
                                    @else
                                    <td></td>
                                    @endif
                                    @if($trans->payment_type == 'COMMISSION' || $trans->payment_type == 'LOAD_WALLET')
                                    <td>{{ $trans->total_amount }}</td>
                                    @else
                                    <td></td>
                                    @endif
