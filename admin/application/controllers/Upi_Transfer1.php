...
class Upi_Transfer extends BaseController {
  ...
  public function doFundTransfer() {    
    ...
    if(!empty($data['operatorID']) && !empty($data['sender_mobile_number'])){
      ...
      //check ampunt more than 25000 start
        if($data['transaction_amount']>25000){
        $response = array(
          'status' => "false",
          'msg' => "Exceed amount",
          'result' => null
      ...
      //check ampunt more than 25000 end
      //check active api for operator end
        ...
        $transduplicatecheck=$this->moneyTransferApi_model->checkduplicate_transaction($transdate,$wherecheck);
     ...
     if($api_details->api_id == "12"){//paytm
        ...
        //  $output=curl_exec($ch);
        //  curl_close($ch);
         $msg="TID: ".$order_id." Amt : ".$amount." Fees: 1 %, Name : ".$money_re->name." UPI : ".$benificiary_details->bank_account_number." Mobile ".$benificiary_details->recipient_mobile_number." www.smartpaytech.in SMARTPAY";
        ...
        //insert DEBIT txn into tbl_wallet_trans_dtls table
          ...
          //  if($resultsta['data']['status']=="PENDING"){
