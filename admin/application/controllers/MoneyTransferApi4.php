       ...
       //print_r($tarr);
        ...
        //check avalibe limit in sender table end
          ...
          */
          ...
          //UPDATE TRANSACTION TABLE NOW_UPDATING
          $bnk = ( $result1[$i]['statusCode'] == "DE_001" ) ?  $result1[$i]['result']['rrn'] :  $result1[$i]['result']['paytmOrderId'] ;
          
          if($tarr[$j]['statusCode'] == "DE_101" || $tarr[$j]['statusCode'] == "DE_001"){
            $trans_update_arr = array(
              'transaction_status' =>$result1[$i]['status'],
              'bank_transaction_id' =>$bnk,
          ...
          //END UPDATE TRNASACTION TABLE
          // UPDATE API LOG
         

          $update_api_dtls_arr =  array('response' => json_encode($result1[$i]), 'updated_on'=>date('Y-m-d H:i:s') );
          $update_api_log = $this->db->where('order_id', $order_id)->update('tbl_apilog_dts', $update_api_dtls_arr );
          //END UPDATE API LOG
