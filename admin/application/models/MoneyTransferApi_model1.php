...
class MoneyTransferApi_model extends CI_Model
{
    ...
    function getdata_where($table,$where){
        $this->db->select('*');
        $this->db->from($table);
        $this->db->where($where);
        $query = $this->db->get();
    ...
    function checkduplicate_transaction_new($transdate,$where){
         $cDate=date('Y-m-d H:i:s');

       
        print_r($cDate) ;

           $sql="SELECT * from tbl_transaction_dtls WHERE  DATE(trans_date) >= DATE_SUB('".$cDate."', INTERVAL 5 MINUTE) AND DATE(trans_date) < '".$cDate."' And service_id='".$where['service_id']."' AND operator_id='".$where['operator_id']."' AND recipient_id='".$where['recipient_id']."' AND request_amount='".$where['request_amount']."'"; 
   ...
   function get_senderdts_byaccno($accno){
      $this->db->select('t2.sender_mobile_number,t2.available_limit,t2.used_limit,t2.Upi_available_limit,t2.Upi_used_limit,CONCAT(t2.first_name, '.', t2.last_name) as sender_name', FALSE);
        $this->db->from('tbl_dmt_benificiary_dtls as t1');
